...
def test_parallel_allreduce(shape, comp_stream, comm_stream):
    ...
    with Device("cuda(0)"):
        ...
        mod = RAFSequential([EnforceSync(1)])(mod)
    ...
    with Device("cuda(0)"):
        ...
        def expected():
            ...
            fn (%x: Tensor[(64, 128), float32]) {
                ...
                let %set_stream_comm = raf.op.set_stream(int64(0), int64(4));
                let %wait_for_comp = raf.op.wait_event(int64(1), int64(4));
                let %v2 = raf.op._allreduce(%v1, str"sum");
                let %set_stream_comp1 = raf.op.set_stream(int64(0), int64(1));
                ...
                let %set_stream_comm1 = raf.op.set_stream(int64(0), int64(4));
                let %wait_for_comp1 = raf.op.wait_event(int64(2), int64(4));
                let %v5 = raf.op._allreduce(%v4, str"sum");
                let %add_event_comm = raf.op.add_event(int64(3), int64(4));
                let %set_stream_comp2 = raf.op.set_stream(int64(0), int64(1));
                let %wait_for_comm = raf.op.wait_event(int64(3), int64(1));
...
def test_multi_input_allreduce(shape, comp_stream, comm_stream):
    ...
    with Device("cuda(0)"):
        ...
        def expected():
            ...
            builder.wait_event(1, fuse_tensor_stream)
            fused_x_2 = builder.call("fuse_tensor", [x_2])
            builder.add_event(12, fuse_tensor_stream)
            builder.set_stream(0, comm_stream)
            builder.wait_event(1, comm_stream)
            ...
            x_6_a = builder.call("atan", [x_5])
            x_6_b = builder.call("atan", [x_5])
            x_7 = builder.make_tuple([x_6_a, x_6_b])
            builder.add_event(3, comp_stream)
            builder.set_stream(0, fuse_tensor_stream)
            builder.wait_event(3, fuse_tensor_stream)
            fused_x_7 = builder.call("fuse_tensor", [x_7])
            builder.add_event(10, fuse_tensor_stream)
            builder.set_stream(0, comm_stream)
            builder.wait_event(10, comm_stream)
            fused_x_8 = builder.call("_reduce", [fused_x_7, raf.ir.const(0), raf.ir.const("sum")])
            builder.add_event(9, comm_stream)
            builder.set_stream(0, defuse_tensor_stream)
            builder.wait_event(9, defuse_tensor_stream)
            x_8 = builder.call("defuse_tensor", [fused_x_8, raf.ir.const(sizes),
                                                 raf.ir.const(tuple_shape), raf.ir.const(indices)])
            builder.add_event(4, defuse_tensor_stream)
            builder.set_stream(0, comp_stream)
            builder.wait_event(4, comp_stream)
            x_9_a = builder.get_tuple_item(x_8, 0)
            x_9_b = builder.get_tuple_item(x_8, 1)
            x_10 = builder.call("multiply", [x_9_a, x_9_b])

            x_11_a = builder.call("atan", [x_10])
            x_11_b = builder.call("atan", [x_10])
            x_12 = builder.make_tuple([x_11_a, x_11_b])
            builder.add_event(5, comp_stream)
            builder.set_stream(0, fuse_tensor_stream)
            builder.wait_event(5, fuse_tensor_stream)
            fused_x_12 = builder.call("fuse_tensor", [x_12])
            builder.add_event(8, fuse_tensor_stream)
            builder.set_stream(0, comm_stream)
            builder.wait_event(8, comm_stream)
            fused_x_13 = builder.call("_broadcast", [fused_x_12, raf.ir.const(0)])
            builder.add_event(7, comm_stream)
            builder.set_stream(0, defuse_tensor_stream)
            builder.wait_event(7, defuse_tensor_stream)
            x_13 = builder.call("defuse_tensor", [fused_x_13, raf.ir.const(sizes),
                                                  raf.ir.const(tuple_shape), raf.ir.const(indices)])
            builder.add_event(6, defuse_tensor_stream)
            builder.set_stream(0, comp_stream)
            builder.wait_event(6, comp_stream)
            x_14_a = builder.get_tuple_item(x_13, 0)
            x_14_b = builder.get_tuple_item(x_13, 1)
            x_15 = builder.call("multiply", [x_14_a, x_14_b])

            return tvm.relay.Function([x], builder.ret(x_15))
...
def test_dependency_analysis(
    ...
    with Device("cuda(0)"):
        ...
        with PassContext(config={"raf.annotate_collective_ops.use_memory_copy_ops": True}):
            mod = tvm.IRModule()
            mod["main"] = construct_model_func()
            mod = RAFSequential([AnnotateCollectiveOps(), EnforceSync(1)])(mod)
...
if __name__ == "__main__":
    ...
    test_multi_input_collective_ops_with_otf_copy((64, 128), 1, 4, 5, 6)
