...
parser = argparse.ArgumentParser(description='Tune tvm softmax dx op.')
parser.add_argument('--shape', required=True, type=int, nargs="+", help='Input shape')
parser.add_argument('--output', '-o', type=str, default="./tvm_softmax_dx_schedule.json",
...
def tune_tvm_softmax_dx(shape, out_path, dtype):
    with Device("cuda(0)"):
        def test():
            builder = ANFBuilder()
            x = extended_var("x", shape=shape, dtype=dtype)
            y = extended_var("y", shape=shape, dtype=dtype)
            dy = extended_var("dy", shape=shape, dtype=dtype)
            out = builder.call("softmax_dx", [x, y, dy])
            return tvm.relay.Function([x, y, dy], builder.ret(out))

        x_np = np.random.rand(*shape).astype(dtype)
        x = raf.array(x_np, device="cuda(0)")

        y_np = np.random.rand(*shape).astype(dtype)
        y = raf.array(y_np, device="cuda(0)")
        ...
        # tune the op
        run_tuning(test_mod, "cuda", [x, y, dy], out_path, n_trials=lambda l: 300 * min(l, 100))

if __name__ == "__main__":
    args = parser.parse_args()
    input_shape = tuple(args.shape)
    print("Tuning with shape: ", input_shape)
    tune_tvm_softmax_dx(input_shape, args.output, "float32")
