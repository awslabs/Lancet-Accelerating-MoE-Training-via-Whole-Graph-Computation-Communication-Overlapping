...
class Connector {
 public:
  ...
  virtual void Gather(void* send_buffer, int send_count, void* recv_buffer, int recv_count, int root) = 0;
  virtual void Alltoall(void* send_buffer, int per_worker_send_count, void* recv_buffer) = 0;
...
class ConnectorManager {
 public:
  ...
  Connector* GetConnector(const std::string& name) {
    ...
    std::string conn_name = conn_->GetType();
    std::transform(conn_name.begin(), conn_name.end(), conn_name.begin(),
                [](unsigned char c){ return std::tolower(c); });
    CHECK(name == "" || name == conn_name) << "You have already initialized a connector [" << conn_->GetType()
                       << "], but is requesting a connector [" << name << "]. Currently we do not support multiple connectors";
    return conn_.get();
  }
