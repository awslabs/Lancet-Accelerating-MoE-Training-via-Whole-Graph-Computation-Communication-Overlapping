...
/*!
 * \brief A pass that applies the backward function (backward closure remains).
 * \return The created pass.
 */
Pass ApplyBackward();
...
ir::Expr AssignDeviceForExpr(ir::Expr expr, std::string device);
...
ir::Expr EraseType(const ir::Expr& expr, bool is_main=true);

ir::Function DelayAllToAllvInFunction(ir::Function f);
