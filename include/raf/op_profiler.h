...
class OpProfiler {
 public:
  ...
  /*!
  * \brief Dump latency cache in text format.
  */
  std::string DumpLatencyCache();

  /*!
  * \brief Load latency cache from text format.
  */
  void LoadLatencyCache(const std::string& cache_str);

  void SetLatencyCacheValue(const std::string& key, const std::vector<float>& latency,
  ...
  HashKey HashCall(const Call& call) {
    ...
    if (auto op_node = call->op.as<OpNode>()) {
      ...
      if (fn_node->HasNonzeroAttr(attr::kPrimitive)) {
        // use structual hash as key
        Function func = Downcast<Function>(call->op);
        key << tvm::StructuralHash()(func);
      } else {
    ...
    for (auto arg : call->args) {
      if(arg.as<ConstantNode>() && arg.as<ConstantNode>()->IsScalar()) {
        key << raf::ir::AsText(arg);
      } else {
