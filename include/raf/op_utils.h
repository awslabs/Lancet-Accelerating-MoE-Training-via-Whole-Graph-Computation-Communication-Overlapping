...
inline bool IsReshapeOp(const Expr& op) {
  if (auto op_node = op.as<OpNode>()) {
    return IsReshapeOp(GetRef<Op>(op_node));
  }
  return false;
}

inline bool IsUpdateOp(const Op& op) {
  static std::unordered_set<Op, ObjectPtrHash, ObjectPtrEqual> update_ops{
      Op::Get("raf.op.add"), Op::Get("raf.op.subtract")};
  auto op_ = IsDialectOp(op) ? GetBaseOp(op) : op;
  return update_ops.find(op_) != update_ops.end();
}
...
inline const Op GetDialectOp(const std::string& op_name) {
