...
def extract_tuning_tasks(mod_or_executor, args, device, *, fusion=False, pass_seq=None, tracing_mode="complex"):
    ...
    tracing_mode = auto_scheduler.relay_integration.TracingMode.EXTRACT_COMPLEX_TASK_ONLY if tracing_mode == "complex" else auto_scheduler.relay_integration.TracingMode.EXTRACT_TASK
    ...
    if "(" in device:
        tvm_target_device = device.split("(")[0]
    else:
        tvm_target_device = device
    tvm_target = tvm.target.Target(tvm_target_device)
...
def tune_tasks(tasks, weights, log_file, n_trials, device_id=0):
    ...
    """
    measure_device = auto_scheduler.LocalRPCMeasureContext(repeat=1, min_repeat_ms=400, timeout=10, device=device_id)
...
def run_tuning(
    ...
    *,
    ...
    tasks, weights = extract_tuning_tasks(
        model_or_executor, args, device, fusion=fusion, pass_seq=pass_seq, tracing_mode=tracing_mode
    )
    ...
    print("Tuning %d out of %s tasks on device %s..." % (len(tasks), ori_task_num, device))
    if "(" in device:
        device_id = int(device.split("(")[1].split(")")[0])
        tune_tasks(tasks, weights, log_file, n_trials, device_id=device_id)
    else:
