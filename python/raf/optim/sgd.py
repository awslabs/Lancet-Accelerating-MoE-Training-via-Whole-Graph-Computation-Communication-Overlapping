    ...
    def decorator(model):
        ...
        class SGDWrapper(Model):
            ...
            def build(self, model):
                nonlocal fw_only_model
                self.model = model
                self.ad_model, fw_only_model = with_data_parallel(with_autodiff(model))
                ...
                for name, param in self.model.state().items():
                    ...
                    if param.requires_grad is True:
                        ...
                        # print("name: ", name, "shape: ", status_shape)
                        if dctx.zero_opt_level and len(status_shape):
                            # If optimizer status partitioning is enabled, then the first axis of
                            # variant and weight is partitioned to 1/n. Accordingly, we have to
                            # also keep a param.w (size 1/n) locally.

                            # Pad and copy a slice of weight to be the SGD status.

                            # Should not slice all2all params, while in build function we cannot do this.
                            # Will correct this in forward function.
                ...
                self.learning_rate = array(learning_rate, dtype='float32', device=device)
                self.momentum = array(momentum, dtype='float32', device=device)

                # TODO(issue 758): Remove this and in-place update parameters.
                if self.has_sgd_w:
                    self.zero = array(0, dtype=self.dtype, device=device)
            ...
            def forward(self, dy, *args, **kwargs):
                ...
                dctx = dist.get_context()
                expert_params_map = []
                if hasattr(self.ad_model, "_record"):
                    record_ = getattr(self.ad_model, "_record")
                    if "expert_params_map" in record_.param_attributes:
                        expert_params_map = record_.param_attributes["expert_params_map"]

                states = self.model.state()
                for i, param in enumerate(inputs):
                    ...
                    if param in self.params and has_grad(dxi):
                        ...
                        # the updated weights.
                        if dctx.zero_opt_level > 0 and len(sgd_w.shape) and name not in expert_params_map:
        ...
        return SGDWrapper(model), fw_only_model
