...
from raf.ir import RAFSequential, AsText
...
from .._ffi.pass_ import AutoDiff, InlineBackward, ApplyBackward, Substitute, InferType, FoldConstant, SeparateForward, GradientInputSelection
...
def with_autodiff(model):
    ...
    class AutoDiffWrapper(Model):
        ...
        def forward(self, dy, *args, **kwargs):
            ...
            if not hasattr(self, "_record"):
                if hasattr(self.model, "_record"):
                    setattr(self, "_record", getattr(self.model, "_record"))
                else:
                    setattr(self, "_record", record)
            dy = calc_dy(dy, record)
            mod = record.mod
            passes = [InferType(), AutoDiff(record.requires_grads)]
            if get_context().enable_data_parallel:
                if "is_expert_parallel" in record.param_attributes:
                    is_expert_parallel = record.param_attributes["is_expert_parallel"]
                else:
                    is_expert_parallel = []
                passes.append(AutoDataParallel(is_expert_parallel))
            passes += [InferType(), FoldConstant(), DeadCodeElimination()]
            if get_context().overlap_comm_forward:
                # inline backward delayed after adding optimizer
                print("[INFO] Using ApplyBackward instead of InlineBackward.")
                passes.append(GradientInputSelection())
                passes.append(ApplyBackward())
            else:
                passes.append(InlineBackward())
    ...
    class SeparateForwardWrapper(Model):
        """AutoDiff model
        ...
        def forward(self, dy, *args, **kwargs):
            ...
            if get_context().enable_data_parallel:
                if "is_expert_parallel" in record.param_attributes:
                    ...
                    is_expert_parallel = []
                passes.append(AutoDataParallel(is_expert_parallel))
            passes += [InferType(), FoldConstant(), DeadCodeElimination(), GradientInputSelection()]
            passes.append(SeparateForward())
            seq = RAFSequential(passes)
            mod = seq(mod)
            inputs = _get_func_inputs(record, args, kwargs)
            out = inline(mod["main"], inputs)
    ...
    if get_context().overlap_comm_forward:
        return AutoDiffWrapper(model), SeparateForwardWrapper(model)
    else:
        return AutoDiffWrapper(model), None
