...
def _get_vm_executor(mod, device, opt_level=2, disable_fusion=False, **options):
    ...
    options.setdefault("partition_comm", False)

    config = {
        "raf.stream_schedule.policy": options["stream_schedule_policy"],
        "raf.vm.optimize.anf_only": options["anf_only"],
        "raf.dp_schedule.partition_large_comm": options["partition_comm"]
    }
...
def run_vm_model(model, device, args, opt_level=2, disable_fusion=False, **options):
    ...
    return run_trace_record(record, device, args, kwargs, opt_level, disable_fusion, **options)


def run_trace_record(record, device, args, kwargs={}, opt_level=2, disable_fusion=False, **options):
    """Helper function to execute record with VM"""
    inputs = _get_func_inputs(record, args, kwargs, get_handle=False)
    vm = get_vm_executor(record.mod, device, opt_level, disable_fusion, **options)
    out = vm(*inputs)
    return out
