...
class VMExecutor:
    ...
    def _make_vm_helper(maker, sch_file=None):
        ...
        def _vm_wrapper(*args, **kwargs):
            ...
            allow_jit_failure = tvm.transform.PassContext.current().config.get(
               "raf.tvm.allow_jit_failure", False
            )
            if allow_jit_failure:
                config["raf.tvm.allow_jit_failure"] = True
