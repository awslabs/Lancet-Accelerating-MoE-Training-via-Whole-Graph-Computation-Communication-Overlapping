...
using LinkNode = tvm::relay::LinkNode<Node*>;
using NodeExprMap = std::unordered_map<const Node*, Expr>;

/*! Try add edge to graph if not exist. Returns whether new edge is inserted.*/
bool AddGraphEdge(Node* parent, Node* child, Arena* arena) {
  // check if the edge exists currently 
  bool link_exists = false;
  for(auto child_parent = child->parents.head; child_parent; child_parent = child_parent->next) {
    if (child_parent->value == parent) {
      link_exists = true;
      break;
    }
  }
  if(!link_exists) {
    auto* parent_link = arena->make<LinkNode>();
    parent_link->value = parent;
    child->parents.Push(parent_link);
    auto* child_link = arena->make<LinkNode>();
...
void RemoveGraphNode(DependencyGraph* dg, std::unordered_set<Node*> nodes) {
  std::vector<std::pair<Node*, Node*>> edges2remove;
  NodeExprMap node_expr;
  for(auto node_it: dg->expr_node) {
    node_expr[node_it.second] = node_it.first;
  }
  for(auto node: nodes) {
    ...
    for (auto child = node->children.head; child; child = child->next) {
      edges2remove.emplace_back(node, child->value);
    }
  }
  std::vector<Node*> new_order;
  for (auto n : dg->post_dfs_order) {
    if (!nodes.count(n)) {
  ...
  for(auto node: nodes) {
    if (node_expr.count(node) && dg->expr_node.count(node_expr.at(node))) {
      dg->expr_node.erase(node_expr.at(node));
    }
  }
}
...
// void GetTopologicalOrder(DependencyGraph* dg, std::vector<Node*>& nodes) {
//   std::unordered_map<Node*, int> node_idx = {};
//   for (int i = 0; i < dg->post_dfs_order.size(); ++i) {
//     node_idx[dg->post_dfs_order[i]] = i;
//   }
//   std::sort(nodes.begin(), nodes.end(), [&](Node* lhs, Node* rhs) {
//     return node_idx[lhs] < node_idx[rhs];
//   });
// }

// void GetTopologicalOrder(DependencyGraph* dg, std::vector<const Node*>& nodes) {
//   std::unordered_map<const Node*, int> node_idx = {};
//   for (int i = 0; i < dg->post_dfs_order.size(); ++i) {
//     node_idx[(const Node*) dg->post_dfs_order[i]] = i;
//   }
