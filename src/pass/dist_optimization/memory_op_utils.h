...
static FuseAndDefuseOpToAddMap fuse_and_defuse_op_to_add_map = {
    ...
    {Op::Get("raf.op._reduce_scatter"), {FuseOp::kFuseReorder, DefuseOp::kDefuse}},
...
using FuseReorderTensorArgs = std::tuple<int>;

// For ReduceScatter
FuseReorderTensorArgs GetFuseReorderTensorArgs(const CallNode* call) {
  TupleType tt = Downcast<TupleType>(call->args[0]->checked_type_);
  ...
  auto shapes = ArrayToInt(Downcast<TupleValue>(call->args[1])->fields);
  auto shape_indices = ArrayToInt(Downcast<TupleValue>(call->args[2])->fields);
  size_t start_idx = 0;
  int64_t total_partitioned_size = 0;
  for (int i = 0; i < shape_indices.size(); ++i) {
    int64_t partitioned_size = 1;
    for (auto it = shapes.begin() + start_idx; it != shapes.begin() + shape_indices[i]; ++it) {
      partitioned_size *= *it;
    }
    total_partitioned_size += partitioned_size;
    start_idx = shape_indices[i];
  }
  CHECK(total_size % total_partitioned_size == 0) << "Cannot partition tensors of total size " <<
    total_size << " into several parts of size " << total_partitioned_size << ".";
  int n_parts = total_size / total_partitioned_size;
  FuseReorderTensorArgs args = std::make_tuple(n_parts);
  return args;
}
