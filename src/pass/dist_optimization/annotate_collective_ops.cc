...
class CollectiveOpAnalyzer : ExprVisitor {
 public:
  ...
  // the set of ops which needs a fuse_reorder_tensor before it
  std::unordered_set<int> fuse_reorder_tensor_before_op;
  // arguments of fuse_reorder_tensor
  std::unordered_map<int, FuseReorderTensorArgs> fuse_reorder_tensor_args;
 ...
 private:
  void UpdateOpToAddInfo_(const CallNode* call) {
    ...
    switch (fuse_op_to_add) {
      case FuseOp::kFuse:
        fuse_tensor_before_op.insert(current_idx_);
        break;
      case FuseOp::kFuseReorder:
        fuse_reorder_tensor_before_op.insert(current_idx_);
        fuse_reorder_tensor_args[current_idx_] = GetFuseReorderTensorArgs(call);
        break;
      case FuseOp::kNone:
      default:
        break;
    }
    switch (defuse_op_to_add) {
      case DefuseOp::kDefuse:
        defuse_tensor_after_op.emplace(current_idx_);
        defuse_tensor_args[current_idx_] = GetDefuseTensorArgs(call);
        break;
      case DefuseOp::kNone:
      default:
        break;
...
class CollectiveOpAnnotator : ExprVisitor {
 public:
  ...
  void VisitExpr_(const LetNode* op) {
    ...
    if (orig_value->IsInstance<CallNode>() && IsCollectiveOp(orig_value.as<CallNode>()->op)) {
      ...
      else if (analyzer_.fuse_reorder_tensor_before_op.count(orig_value_idx)) {
        // add fuse reorder tensor before it
        Var orig_value_var = GetRef<Var>(orig_value.as<CallNode>()->args[0].as<VarNode>());
        std::string name_hint = "fuse_reordered_";
        name_hint += std::string(orig_value_var->name_hint());
        FuseReorderTensorArgs fuse_reorder_args = analyzer_.fuse_reorder_tensor_args[orig_value_idx];
        Var fuse_reordered_orig_value_var = raf::ir::MakeVar(name_hint, {});
        Expr fuse_reordered_tensor_value = CreateFuseReorderTensorOp(orig_value_var, fuse_reorder_args);
        ell_->Push(fuse_reordered_orig_value_var, fuse_reordered_tensor_value);

        std::string fused_tuple_name_hint = "fuse_reordered_tuple_";
        fused_tuple_name_hint += std::string(orig_value_var->name_hint());
        Var fuse_reordered_tuple_orig_value_var = raf::ir::MakeVar(fused_tuple_name_hint, {});
        ell_->Push(fuse_reordered_tuple_orig_value_var, Tuple({fuse_reordered_orig_value_var}));

        Array<Expr> args;
        args.push_back(fuse_reordered_tuple_orig_value_var);
        for (int i = 1; i < orig_value.as<CallNode>()->args.size(); ++i) {
          args.push_back(orig_value.as<CallNode>()->args[i]);
        }
        substituded_value = Call(orig_value.as<CallNode>()->op, args);
 ...
 protected:
  ...
  Expr CreateFuseReorderTensorOp(Expr arg, const FuseReorderTensorArgs& args) {
    static Op op = Op::Get("raf.op.fuse_reorder_tensor");
    Expr n_part = MakeConstant(ScalarValue::make(std::get<0>(args)));
    Array<Expr> args_({arg, n_part});
