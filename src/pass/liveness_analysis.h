...
using MapStreamVSet = StdMap<StreamVSet>;
...
class LivenessAnalyzer {
 public:
  ...
  StreamVSet GetLiveVars(const Var& x) {
    if (live_.count(x) == 0) {
      return StreamVSet();
    }
    return live_.at(x);
  ...
  StreamVSet GetLiveLines(const Var& x) {
    StreamVSet ret;
    if (inv_live_.find(x) == inv_live_.end()) {
      // try to get dummy var
      auto dummy_vars = GetTensorVars(x);
      for (auto dummy_var : dummy_vars) {
        CHECK(inv_live_.find(dummy_var) != inv_live_.end());
        for (const auto& kv : inv_live_.at(dummy_var)) {
          if (kv.second.size() > 0) {
            ret[kv.first].insert(kv.second.begin(), kv.second.end());
          }
        }
      }
    } else {
      for (const auto& kv : inv_live_.at(x)) {
        if (kv.second.size() > 0) {
  ...
  StreamVSet GetOutputTensorVars() {
  ...
  Var Unite(const Var& x, const Var& y) {
    ...
    for (auto u : inv_live_.at(fx)) {
      inv_live_[fy][u.first].insert(u.second.begin(), u.second.end());  
    }
    return fy;
  }
  ...
  bool Intersect(const Var& x, const Var& y) {
    ...
    const StreamVSet& sx = inv_live_.at(x);
    const StreamVSet& sy = inv_live_.at(y);
    for (const auto& v : sx) {
      if (sy.find(v.first) != sy.end()) {
        for (const auto& u : v.second) {
          if(sy.at(v.first).find(u) != sy.at(v.first).end()) {
            return true;
          }
        }
      }
  ...
  std::unordered_set<int> GetStreams() {
    std::unordered_set<int> streams;
    for (const auto& kv : live_) {
      for (const auto& v : kv.second) {
        streams.insert(v.first);
      }
    }
    return streams;
  }

  static std::string DebugDump_(VSet vset) {
  ...
  static std::string DebugDump_(StreamVSet vset) {
    std::ostringstream os;
  ...
  static std::string DebugDump_(MapStreamVSet vset, Var x) {
    std::ostringstream os;
    os << x << "(vid: " << x->vid << "): \n";
    if (vset.find(x) != vset.end()) {
      const StreamVSet& vs = vset.at(x);
      for (const auto& v : vs) {
        os << "\tStream " << v.first << ": [";
        for (const auto& u : v.second) {
          os << u << "(vid: "<< u->vid << "), ";
  ...
  static std::string DebugDump_(MapVSet vset, Var x) {
    std::ostringstream os;
    os << x << ": ";
    if (vset.find(x) != vset.end()) {
  ...
  static std::string DebugDump(VSet vset) {
    return DebugDump_(vset);
  }

  static std::string DebugDump(StreamVSet vset) {
    return DebugDump_(vset);
  }

  /*! \brief Debug output: vset_ */
  static std::string DebugDump(MapStreamVSet vset, Var x = Var()) {
    std::ostringstream os;
    if (x.defined()) {
  ...
  static std::string DebugDump(MapVSet vset, Var x = Var()) {
    std::ostringstream os;
    if (x.defined()) {
      return DebugDump_(vset, x);
    }
...
class LivenessAnalyzer::BackwardAnalyzer : public ExprVisitor {
 ...
 private:
  /*! \brief returns live_[next_var_] - vset_[def] + vset_[cur]
             ...
             live(l + 1, x) && !define(l, x) => live(l, x) */
  StreamVSet MergeLive(const Var& cur, const Var& def = Var()) {
    CHECK(analyzer_->live_.find(next_var_) != analyzer_->live_.end());
    StreamVSet ret_vset;
    // LOG(INFO) << "MergeLive: cur = " << cur << ", def = " << def << ", next_var_ = " << next_var_;
    for (auto stream: streams_) {
      Var next_line_var = analyzer_->CreateTensorVar("ml");
      if (analyzer_->live_.at(next_var_).count(stream)) {
        // LOG(INFO) << "MergeLive: found stream = " << stream << ", vset: " << DebugDump(analyzer_->live_.at(next_var_).at(stream));
        analyzer_->vset_[next_line_var] = analyzer_->live_.at(next_var_).at(stream);
      } else {
        analyzer_->vset_[next_line_var] = {};
      }
      ...
      if (stream == current_stream_) {
        ...
        ret_vset[stream] = analyzer_->vset_.at(ret);
      } else {
        ret_vset[stream] = analyzer_->vset_.at(remain);
      }
    }
    // LOG(INFO) << "MergeLive: ret_vset: " << DebugDump(ret_vset);
    return ret_vset;
  }
 ...
 private:
  ...
  /*! \brief the explicit let list of func_ */
  std::unique_ptr<ExplicitLetList> ell_{nullptr};
  /*! \brief all streams used */
  std::unordered_set<int> streams_;
  /*! \brief a variable recording current stream id */
