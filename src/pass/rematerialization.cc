...
class Rematerializer : public ExprMutator {
 public:
  ...
  Expr VisitExpr_(const CallNode* node) {
    auto scope = scopes_.back().get();
    auto streamvset = analyzer_->GetLiveVars(curr_let_);
    VSet live_in_vars;
    for (auto var : streamvset) {
      live_in_vars.insert(var.second.begin(), var.second.end());
    }
 ...
 private:
  ...
  Var Rematerialize(LetList* scope, const Var& var, size_t curr_depth = 1) {
    ...
    CHECK(call_node != nullptr) << "Tensor " << latest_let_var
