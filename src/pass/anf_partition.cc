...
using LiveVarSet = std::unordered_set<Var, ObjectPtrHash, ObjectPtrEqual>;
 ...
 public:
  ...
  void TrimOutputs(liveness_analysis::LivenessAnalyzer& analyzer, const Var& next_var,
    ...
    if (is_final_ret) {
      ...
      std::unordered_map<int, LiveVarSet> live_var =
          analyzer.GetLiveVars(next_var);
      LiveVarSet flattened_live_var;
      for (auto& it: live_var) {
        for (auto& var: it.second) {
          flattened_live_var.insert(var);
        }
      }
      std::vector<Expr> trimmed_outs;
      for (auto out_expr : outs_) {
        Var out = Downcast<Var>(out_expr);
        if (analyzer.IsAlive(out, flattened_live_var)) {
