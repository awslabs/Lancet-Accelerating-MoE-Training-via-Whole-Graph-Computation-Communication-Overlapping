...
IRModule RAFSequentialNode::operator()(IRModule mod, const PassContext& pass_ctx) const {
  for (const Pass& pass : passes) {
    ...
    LOG(INFO) << "Starting to run Pass " << pass->Info()->name << ".";
    mod = pass(std::move(mod), pass_ctx);
    auto fin = std::chrono::system_clock::now();
    LOG(INFO) << "Running Pass " << pass->Info()->name << " took " << std::chrono::duration_cast<std::chrono::milliseconds>(fin - start).count() / 1000.0 << " seconds.";
  }
  return mod;
}
