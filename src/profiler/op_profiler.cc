...
std::pair<std::vector<float>, float> OpProfiler::ProfileOp(const Expr& op, int32_t warmup,
  ...
  if (auto call_node = op.as<CallNode>()) {
    ...
    try {
      cost = RunOp(op_with_data, warmup, exec_number, repeat);
    } catch (const std::exception& e) {
      LOG(WARNING) << "Failed to profile expr: " << op << " error: " << e.what() << ", setting its exec time to 1us.";
      cost = std::vector<float>(repeat, 1.0);
    }
...
std::string OpProfiler::DumpLatencyCache() {
  std::ostringstream ss;
  ss << latency_and_workspace_size_cache_.size() << "|";
  for (auto& kv : latency_and_workspace_size_cache_) {
    ss << kv.first << "|";
    ss << kv.second.first.size() << "|";
    for (auto& latency : kv.second.first) {
      ss << latency << "|";
    }
    ss << kv.second.second << "|";
  }
...
void OpProfiler::LoadLatencyCache(const std::string& cache_str) {
  std::istringstream ss(cache_str);
  std::string s;
  std::getline(ss, s, '|' );
  int workspace_size = std::stoi(s);
  for (int i = 0; i < workspace_size; ++i) {
    std::string key;
    int n_entries;
    std::getline(ss, key, '|');
    std::getline(ss, s, '|');
    n_entries = std::stoi(s);
    ...
    std::getline(ss, s, '|');
    workspace_size = std::stoll(s);
    latency_and_workspace_size_cache_[key] = std::make_pair(latencies, workspace_size);
  }
}

void OpProfiler::SetLatencyCacheValue(const std::string& key, const std::vector<float>& latency,
                          const int64_t workspace_size) {
  latency_and_workspace_size_cache_[key] = std::make_pair(latency, workspace_size);
}
...
RAF_REGISTER_GLOBAL("raf.op_profiler.DumpLatencyCache").set_body_typed([](const Device& device) {
  auto profiler = OpProfiler::Get(device);
  String cache_str = profiler->DumpLatencyCache();
  Array<Integer> c_str;
  for(int i = 0; i < cache_str.size(); ++i) {
    c_str.push_back(Integer(cache_str.at(i)));
  }
  return c_str;
});

RAF_REGISTER_GLOBAL("raf.op_profiler.LoadLatencyCache")
    .set_body_typed([](const Device& device, const Array<Integer>& cache_bytes) {
      auto profiler = OpProfiler::Get(device);
      std::string cache_str;
      for (int i = 0; i < cache_bytes.size(); ++i) {
        cache_str.push_back(cache_bytes[i]);
      }
      return profiler->LoadLatencyCache(cache_str);
    });
