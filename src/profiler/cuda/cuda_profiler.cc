...
void CudaProfilerHelper::collect() {
  ...
  auto cuda_profiler = CudaProfiler::Get(device_.device_id());
...
CudaProfiler::CudaProfiler(int device_id) {
  ...
  if (api->GetDeviceCount() > 1) {
    ...
    LOG(WARNING) << "Creating cuda profiler on device: " << device_id << ".";
  }
...
CudaProfiler* CudaProfiler::Get(int device_id) {
  static CudaProfiler cuda_profiler(device_id);
...
void CudaProfiler::start() {
  ...
  if (!started_) {
    api->WaitDevice(Device(DevType::kCUDA(), device_id_));
...
void CollectCudaProfile() {
  CudaProfiler::Get(device_api::DeviceAPI::Get(DevType::kCUDA())->GetDevice())->CollectCudaStat();
}
...
RAF_REGISTER_GLOBAL("raf.profiler.CudaProfilerStart").set_body_typed(CudaProfilerStart);
RAF_REGISTER_GLOBAL("raf.profiler.CudaProfilerStop").set_body_typed(CudaProfilerStop);
