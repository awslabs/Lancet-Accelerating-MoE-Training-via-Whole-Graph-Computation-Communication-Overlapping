...
Array<Expr> CastGrad(const Expr& orig_call, const Array<Expr> orig_args, const Var& y,
  ...
  const CallNode* call = orig_call.as<CallNode>();
  const Expr& x = call->args[0];
  if(y->checked_type_.defined()) {
    // no gradient if casting to int or bool
    auto y_dtype = y->checked_type().as<TensorTypeNode>()->dtype;
    if(y_dtype.is_bool() || y_dtype.is_int() || y_dtype.is_uint()) {
      return {ir::NullValue<ir::Expr>()};
    }
  }
