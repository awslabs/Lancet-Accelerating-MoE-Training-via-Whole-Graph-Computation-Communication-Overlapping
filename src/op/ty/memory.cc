...
template <class T>
Type AddUpInfer(const CallValues& value) {
...
RAF_OP_TYPE("raf.op.fuse_tensor", "FuseTensor", AddUpInfer<FuseTensorArgs>);

RAF_OP_TYPE("raf.op.fuse_reorder_tensor", "FuseReorderTensor", AddUpInfer<FuseReorderTensorArgs>);

Type CopyInplaceInfer(const CallValues& value) {
  const auto* args = value->args.as<CopyInplaceArgs>();
  CHECK(args != nullptr);
  return GetType(args->dst_data);
}

RAF_OP_TYPE("raf.op.copy_inplace", "CopyInplace", CopyInplaceInfer);
