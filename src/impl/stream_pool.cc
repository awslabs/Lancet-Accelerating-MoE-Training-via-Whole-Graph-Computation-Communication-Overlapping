...
class Stream::Impl {
 public:
  explicit Impl(const Device& dev, bool high_priority=false) : device(dev), api(DeviceAPI::Get(dev.device_type())) {
    this->stream = api->CreateStream(dev, high_priority);
  }
...
class StreamPool {
 public:
  ...
  std::shared_ptr<Stream> GetStream(int tag_index, int index, bool high_priority=false) {
    std::lock_guard<std::mutex> lock(mutex);
    ...
    if (pool[tag_index][index] == nullptr) {
      pool[tag_index][index] = std::make_shared<Stream>(new Stream::Impl(device, high_priority));
    }
...
std::shared_ptr<Stream> Stream::Get(const Device& dev, int tag_index, int index, bool high_priority) {
  return StreamPool::Get(dev)->GetStream(tag_index, index, high_priority);
}
