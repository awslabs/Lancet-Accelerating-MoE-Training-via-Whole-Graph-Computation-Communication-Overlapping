...
tvm::String GetMemoryType(TensorValue value) {
  if (value->mem == nullptr) {
    return "OwnedMemory";
  } else {
    using NORAMemory = memory_pool::no_pool::NoPool::NonOwnedReservedAsyncMemory;
    using NOAMemory = memory_pool::NonOwnedAsyncMemory;
    std::shared_ptr<NOAMemory> noa_mem = std::dynamic_pointer_cast<NOAMemory>(value->mem);
    if (noa_mem) {
      return "NonOwnedAsyncMemory";
    }
    std::shared_ptr<NORAMemory> nora_mem = std::dynamic_pointer_cast<NORAMemory>(value->mem);
    if (nora_mem) {
      return "NonOwnedReservedAsyncMemory";
    }
    return "NonOwnedSyncMemory";
  }
}
...
RAF_REGISTER_GLOBAL("raf.value.GetMemoryType").set_body_typed(GetMemoryType);
