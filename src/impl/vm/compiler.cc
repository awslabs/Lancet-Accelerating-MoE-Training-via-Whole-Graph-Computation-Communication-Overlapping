  ...
  void VisitExpr_(const CallNode* call_node) {
    ...
    if (op.as<OpNode>()) {
      ...
      matcher
          ...
          .Match("raf.op.vm.force_free",
                 [this](const Array<Expr>& args, const Attrs& attrs, const Array<Type>& type_arg) {
                   CHECK_EQ(args.size(), 1);

                   this->VisitExpr(args[0]);
                   auto memory_register = last_register_;
                   Emit(Instruction::ForceFree(memory_register));
                 })
...
void VMCompiler::Lower(IRModule mod, const DeviceMap& device_map) {
  ...
  LOG(INFO) << "Module optimization finished.";
  ...
  if (!pass_ctx->GetConfig("raf.vm.optimize.anf_only", Bool(false)).value()) {
    ...
    if ((*it).second.device_type() == DevType::kCUDA()) {
      if (DistContext::Global()->enable_data_parallel) {
        ...
        pass_seqs.push_back(pass::PartitionCommGNF());
        pass_seqs.push_back(pass::DataParallelSchedule(DistContext::Global()->size));
        // pass_seqs.push_back(pass::AnnotateCollectiveOps());
        pass_seqs.push_back(pass::EnforceSync(DistContext::Global()->size));
      } else {
        auto policy_name =
