...
inline std::shared_ptr<Stream> GetStreamById(const VMContext& ctx, Index device_id,
  ...
  if (ctx->streams[device_id][stream_id] == nullptr) {
    if (stream_id == 0) {
      ...
      if (stream_id == kCudaCommunicate ||
          stream_id == kMemCpyCudaToCuda1 ||
          stream_id == kMemCpyCudaToCuda2) {
        high_priority = true;
      }
      Device device(DevType::kCUDA(), static_cast<int>(device_id));
      ctx->streams[device_id][stream_id] =
          Stream::Get(device, kCudaCompute, static_cast<int>(stream_id), high_priority);
    }
  }
...
VMContext VirtualMachine::PrepareVMContext(const std::string& func_name,
  ...
  auto fcreate_ctx = [&]() {
    ...
    std::unordered_set<TensorValue, ObjectPtrHash, ObjectPtrEqual> input_tensor_values;
    for (size_t i = 0; i < inputs.size(); ++i) {
      if (auto tuple_value_node = inputs[i].as<TupleValueObj>()) {
        for(int j = 0; j < tuple_value_node->fields.size(); j++) {
          auto tv = Downcast<TensorValue>(tuple_value_node->fields[j]);
          if (input_tensor_values.count(tv)) {
            ctx->shared_inputs.insert(tv);
          } else {
            input_tensor_values.insert(tv);
          }
        }
      } else if (auto tensor_value_node = inputs[i].as<TensorValueObj>()) {
        auto tv = Downcast<TensorValue>(inputs[i]);
        if (input_tensor_values.count(tv)) {
          ctx->shared_inputs.insert(tv);
        } else {
          input_tensor_values.insert(tv);
...
void VirtualMachine::SetDevices(const std::vector<Device>& devices) {
  ...
  for (const Device& dev : devices) {
    if (dev.device_type() == DevType::kCUDA()) {
      use_cuda_ = true;
      auto cuda_api = DeviceAPI::Get(DevType::kCUDA());
      cuda_api->SetDevice(dev.device_id());
    }
  }
  if (!use_cuda_) {
...
void VirtualMachine::RunLoop(VMContext& ctx) {
  ...
  if (use_cuda_ && profiler::Profiler::Get()->IsProfiling(1)) {
    profiler::CudaProfiler::Get(ctx->current_device_id)->start();
  }
#endif
  ...
  main_loop:
    ...
    switch (instr.op) {
      ...
      case Opcode::ForceFree: {
        WITH_BASE_PROFILER_LEVEL(2, host_device_, "ForceFree", "VMInstruction", {},
                                 { HandleForceFree(ctx, instr); });
        goto main_loop;
      }
      case Opcode::Free: {
...
void VirtualMachine::HandleForceFree(VMContext& ctx, const Instruction& instr) {
  using NORAMemory = memory_pool::no_pool::NoPool::NonOwnedReservedAsyncMemory;
  RegName reg = instr.free.memory;
  auto reg_val = ctx.ReadRegister(reg);
  std::shared_ptr<NonOwnedAsyncMemory> not_owned_async_memory;
  std::shared_ptr<NORAMemory> nora_memory;
  if (reg_val->IsInstance<StorageValueObj>()) {
    auto storage_val = Downcast<StorageValue>(reg_val);
    not_owned_async_memory = std::dynamic_pointer_cast<NonOwnedAsyncMemory>(storage_val->buffer);
    if (!not_owned_async_memory) {
      // check if using NoPool and is reserved async memory
      nora_memory = std::dynamic_pointer_cast<NORAMemory>(storage_val->buffer);
      CHECK(nora_memory != nullptr) << "ForceFree should only be called on async memory allocated in the memory pool.";
    }
  } else {
    ...
    if (!ctx->shared_inputs.count(tensor_val)) {
      // extra inputs can also be reshaped model args, we ignore in this case
      if(tensor_val->mem) {
        not_owned_async_memory = std::dynamic_pointer_cast<NonOwnedAsyncMemory>(tensor_val->mem);
        if (!not_owned_async_memory) {
          // check if using NoPool and is reserved async memory
          nora_memory = std::dynamic_pointer_cast<NORAMemory>(tensor_val->mem);
          CHECK(nora_memory != nullptr) << "ForceFree should only be called on async memory allocated in the memory pool.";
        }
      }
    }
  ...
  if (not_owned_async_memory) {
    not_owned_async_memory->api->FreeMemoryAsync(not_owned_async_memory->data, not_owned_async_memory->stream);
    // reset data to avoid double free
    not_owned_async_memory->data = nullptr;
  } else if (nora_memory) {
    nora_memory->pool->FreeReservedMemoryAsync(nora_memory->nbytes, nora_memory->data, nora_memory->stream);
    // reset data to avoid double free
    nora_memory->data = nullptr;
  }
  ctx->pc++;
}
...
void VirtualMachine::HandleFree(VMContext& ctx, const Instruction& instr) {
  RegName reg = instr.free.memory;
  auto reg_val = ctx.ReadRegister(reg);
  void* current_stream = utils::GetStreamById(ctx, ctx->current_device_id, ctx->current_stream_id)->data();
  if (reg_val->IsInstance<StorageValueObj>()) {
    auto storage_val = Downcast<StorageValue>(reg_val);
    if (storage_val->buffer != nullptr) {
      storage_val->buffer->SetStream(current_stream);
    }
    ...
    CHECK(reg_val->IsInstance<TensorValueObj>())
        << "Expected StorageValue or TensorValue, but got " << reg_val->GetTypeKey();
    auto tensor_val = Downcast<TensorValue>(reg_val);
    if (tensor_val->mem != nullptr) {
      tensor_val->mem->SetStream(current_stream);
    }
...
void VirtualMachine::HandleInvokeJit(VMContext& ctx, const Instruction& instr) {
  ...
  // std::unordered_set<std::string> op_names_to_dump = {"raf.op.cublas.matmul"};
  // static std::unordered_map<std::string, int> op_name_to_counter;
  // Value callee = ctx.ReadRegister(instr.invoke_jit.op_reg);
  // auto op = callee.as<OpValueObj>();
  // if (op && op_names_to_dump.count(op->op->name)) {
  //   auto stream = utils::GetStreamById(ctx, ctx->current_device_id, ctx->current_stream_id);
  //   cudaError_t e = cudaStreamSynchronize(static_cast<cudaStream_t>(stream->data()));
  //   CHECK(e == cudaSuccess) << "CUDA: " << cudaGetErrorString(e);
  //   Map<String, NDArray> inputs_to_dump;
  //   for (size_t i = 0; i < inputs.size(); ++i) {
  //     std::stringstream ss;
  //     if (inputs[i]->IsInstance<TupleValueObj>()) {
  //       auto tuple_val = Downcast<TupleValue>(inputs[i]);
  //       for (int j=0; j<tuple_val->fields.size(); j++) {
  //         auto val = Downcast<TensorValue>(tuple_val->fields[j]);
  //         auto tvm_val = value::ToTVM(val);
  //         inputs_to_dump.Set("input_" + std::to_string(i) + "_" + std::to_string(j), tvm_val);
  //       }
  //     } else {
  //       auto val = Downcast<TensorValue>(inputs[i]);
  //       auto tvm_val = value::ToTVM(val);
  //       inputs_to_dump.Set("input_" + std::to_string(i), tvm_val);
  //     }
  //   }
  //   if (output->IsInstance<TupleValueObj>()) {
  //     auto out_tuple = Downcast<TupleValue>(output);
  //     for (int j=0; j<out_tuple->fields.size(); j++) {
  //       auto val = Downcast<TensorValue>(out_tuple->fields[j]);
  //       auto tvm_val = value::ToTVM(val);
  //       inputs_to_dump.Set("output_" + std::to_string(j), tvm_val);
  //     }
  //   } else {
  //     inputs_to_dump.Set("output", value::ToTVM(Downcast<TensorValue>(output)));
  //   }
  //   std::string inputs_as_bytes = SaveParams(inputs_to_dump);
  //   std::string file_name= "/models/intermediate_outputs/device_" + std::to_string(ctx->current_device_id) + "_" + op->op->name + "_inouts_" + std::to_string(op_name_to_counter[op->op->name]) + ".bin";
  //   SaveBinaryToFile(file_name, inputs_as_bytes);
  //   op_name_to_counter[op->op->name] += 1;
  // }
...
VirtualMachine::PrepareOpEnv(const VMContext& ctx, const Instruction& instr) {
  ...
  if (auto p = op_env_cache->Get(op_env_cache_key)) {
    ...
    if (op) {
      LOG(INFO) << "Starting to create env for new op " << op->op << " with input " << op_env_cache_key << ".";
      call_values->args = GetOpAttr<FRAFSchema>(op->op, "FRAFSchema")(args);
    } else {
      LOG(INFO) << "Starting to create new fused closure with input " << op_env_cache_key << ".";
    ...
    LOG(INFO) << "The creation of OpEnv " << op_env_cache_key << " took " << std::chrono::duration_cast<std::chrono::milliseconds>(fin - start).count() / 1000.0 << " seconds.";
  }
