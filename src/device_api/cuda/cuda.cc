...
class CUDADeviceAPI final : public DeviceAPI {
 public:
  ...
  static cudaMemPool_t GetCUDAMemoryPool(int dev_id) {
    ...
    cuuint64_t offVal = 0;
    CUDA_CALL(cudaMemPoolSetAttribute(mem_pool, cudaMemPoolReuseAllowOpportunistic, &offVal));
    CUDA_CALL(cudaMemPoolSetAttribute(mem_pool, cudaMemPoolReuseAllowInternalDependencies, &offVal));
    // preallocate
    //size_t free_mem, total_mem;
    //void* ptr;
    //CUDA_CALL(cudaMemGetInfo(&free_mem, &total_mem));
    //CUDA_CALL(cudaMallocFromPoolAsync(&ptr, free_mem / 2, mem_pool, nullptr));
  ...
  void* CreateStream(const Device& dev, bool high_priority) override {
    CHECK_EQ(dev.device_type(), DevType::kCUDA());
    int least_priority, greatest_priority;
    CUDA_CALL(cudaDeviceGetStreamPriorityRange(&least_priority, &greatest_priority));
    CUDA_CALL(cudaSetDevice(dev.device_id()));
    cudaStream_t ret = nullptr;
    if(high_priority) {
      LOG(INFO) << "Created high priority stream with priority: " << greatest_priority;
      CUDA_CALL(cudaStreamCreateWithPriority(&ret, cudaStreamDefault, greatest_priority));
    } else {
      LOG(INFO) << "Created low priority stream with priority: " << least_priority;
      CUDA_CALL(cudaStreamCreateWithPriority(&ret, cudaStreamDefault, least_priority));
    }
    return ret;
  }
